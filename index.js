...
function getCarInfoByIndex(inventory, index) {
  const info = inventory.find((item, index) => index === index)
...
function sortCarInventory(inventory) {
  return inventory.sort(function(a, b) {return (a.car_model > b.car_model) ? 1: (a.car_model < b.car_model) ? -1: 0});
}
...
function getModelYears(inventory) {
  ...
  for (var x = 0; x < inventory.length; x++) {
    car_years.push(inventory[x].car_year)
  }
  return car_years;
}
...
function getOlderCars(inventory, year) {
  const older_cars = []
  for (var x = 0; x < inventory.length; x++) {
    if (inventory[x].car_year <= year) {
      older_cars.push(inventory[x])
    }
...
function getGermanCars(inventory) {
  const germanCars = []
  for (var x = 0; x < inventory.length; x++) {
    if (inventory[x].car_make === "Audi" || inventory[x].car_make === "Mercedes-Benz" || inventory[x].car_make === "Volkswagen" || inventory[x].car_make === "BMW") {
      germanCars.push(inventory[x])
